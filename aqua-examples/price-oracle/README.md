...
```mermaid
    ...
    participant N as Additional Network Peers

    C ->> R: request (coin, currency) oracle from k sources
    par for k sources
        R ->> N: request i-th quote
        N ->> R: return i-th quote
    end
    R ->> R: join k quotes
    R ->> N: process array of point quotes, e.g. calculate mean
    N ->> R: collect process result, e.g., mean
    R ->> C: return process result
```
...
```mermaid
    stateDiagram
      ...
      CurlAdpaterModule --> PriceGetterModule: linked
    PriceGetterModule --> PriceGetterService
    CurlAdpaterModule --> PriceGetterService
```
...
Whether you compiled and deployed services, it's time to use deployed service to create our oracle application. And do do just that with Aqua.